...
A: メンバー限定にするには、コンフィグエディターの「発動条件設定」で **ユーザーの役職 > メンバー(またはそれに準ずるもの)** を設定してください。

#### Q. おみくじの内容は変更しても OK？

A: はい、おみくじの内容は自由に変更していただいて構いません。(変更したことによる責任は負いません)

#### Q. おみくじの文字を大きく・小さくしたい、非表示にしたい、など

A: コンフィグエディターの **キャラクター(カラー設定)** や、 **表示設定** で設定が可能です。ただし、パッケージによっては、設定できない箇所もあります。
...
A: [読み上げ設定](https://onecomme.com/docs/settings/speech) をご覧ください。
...
A: わんコメでコメントが受信できていないと、おみくじも反応しません。詳しくは、[Q.コメントがうまく表示されない](https://onecomme.com/docs/faq#%E3%82%B3%E3%83%A1%E3%83%B3%E3%83%88%E9%96%A2%E9%80%A3) をご覧ください。
